...
def load_rul_data():
    ...
    # shape: (14000, 14, 30) 14000时间切片，14个dataslice，
    sample_len = 30
...
def dtw_kmeans_clustering(raw_data, max_clusters=10, use_dtw=True):
    n_instances, n_sensors, window_size = raw_data.shape  # shape: (30, 14, 30)
    sensor_features = raw_data.transpose(1, 0, 2)  # shape: (14, 30, 30)
    instance_features = sensor_features.reshape(n_sensors, -1)
    # instance_features = raw_data.reshape(n_instances, -1)  # shape: (30, 14*30)
...
def build_sfg_neo4j(sensor_instances, cluster_labels, causality_matrix):
    ...
    for si in sensor_instances:
        ...
        flattened_slice = si['slice'].flatten().tolist()  # 展平为 (14*10,) 的列表
    ...
    for i in range(causality_matrix.shape[0]):
        for j in range(causality_matrix.shape[1]):
            if causality_matrix[i, j] == 1:
                w_caus = 2 / np.pi * np.arctan(causality_matrix[i, j])
...
if __name__ == "__main__":
    ...
    raw_data = load_rul_data() # shape: (30, 14, 30)
